...
/**
 * Konservasi Tanaman columns (mirrors pertanian):
 *   0: komoditas (text)
...
export const list = async (q: TableQueryParams) => {
  ...
  const rows = await pool.query<RowDb>(
    `SELECT * FROM konservasi_tanaman ${clause} ${order}
     LIMIT $${params.length + 1} OFFSET $${params.length + 2}`,
    dataParams
  );
  const countRes = await pool.query<{ c: string }>(
    `SELECT COUNT(*)::text AS c FROM konservasi_tanaman ${clause}`,
    params
  );
...
export const listPublic = async (
  ...
  const rows = await pool.query<RowDb>(
    `SELECT * FROM konservasi_tanaman ${where}
     ORDER BY konservasi_tanaman_id ASC
     LIMIT $${params.length - 1} OFFSET $${params.length}`,
    params
  );
  const countRes = await pool.query<{ c: string }>(
    `SELECT COUNT(*)::text AS c FROM konservasi_tanaman ${where}`,
  ...
  for (const row of rows) {
    ...
    const result = await pool.query<{ row_uuid: string }>(
      ...
      [
        ...
        Number(m.get(4) ?? 0),
        String(m.get(5) ?? "Kg"),
        m.get(6) === null ? null : String(m.get(6) ?? "") || null,
        userId,
      ]
    );
...
export const patch = async (
  ...
  for (const row of rows) {
    if (row.colValues === null) {
      await pool.query(`DELETE FROM konservasi_tanaman WHERE row_uuid = $1`, [row.rowId]);
      continue;
    }
    const sets: string[] = [];
    const params: unknown[] = [];
    ...
    const res = await pool.query(
      `UPDATE konservasi_tanaman SET ${sets.join(", ")} WHERE row_uuid = $${params.length}`,
      params
    );
    if (res.rowCount === 0) {
