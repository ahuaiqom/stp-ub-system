...
const FIELDS: FormFieldDef[] = [
  ...
  { colIdx: 2, key: "masa_tanam_bulan",     label: "Masa Tanam (Bulan)", type: "number" },
  { colIdx: 3, key: "masa_tanam_per_tahun", label: "Per-Tahun (kali)",   type: "number" },
  { colIdx: 4, key: "proyeksi_panen",       label: "Proyeksi Panen",     type: "number" },
...
const buildPayload = (v: FormValues) =>
  FIELDS.map((f) => {
    ...
    if (!value && f.key === "keterangan") value = null;
...
const KonservasiTanamanTab = ({ addTrigger }: Props) => {
  const { token } = useAuth();
  ...
  const columns: ColumnDef<TableRow>[] = [
    ...
    { key: "satuan_bulan", label: "Satuan Bulan", align: "center", render: (r) => String(getCol(r, 2) ?? "") },
    { key: "per_tahun", label: "Per-Tahun (kali)", align: "center", render: (r) => String(getCol(r, 3) ?? "") },
    { key: "proyeksi", label: "Proyeksi Panen", align: "center", render: (r) => String(getCol(r, 4) ?? "") },
  ...
  return (
    <>
      <DataTable
        title="Tabel Konservasi Tanaman"
        columns={columns}
        rows={t.rows}
        loading={t.loading}
        error={t.error}
      ...
      <RowFormModal
        open={showAdd}
        title="Tambah Konservasi Tanaman"
        fields={FIELDS}
...
export default KonservasiTanamanTab;
